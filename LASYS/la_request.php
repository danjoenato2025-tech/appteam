...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['__action'])) {
    ...
    switch ($_POST['__action']) {
        ...
        case 'get_next_request_number':
            $year = date('y');
            $prefix = "S-LA-{$year}-";
            $stmt = $pdo->prepare("SELECT request_number FROM la_request WHERE request_number LIKE ? ORDER BY record_id DESC LIMIT 1");
            $stmt->execute([$prefix . '%']);
        ...
        case 'add_record':
            ...
            $sts = trim($_POST['status'] ?? '') ?: null;
            if (!$dr || !$rn) {
                echo json_encode(['success' => false, 'message' => 'Date Requested and Request Number are required.']);
                exit;
            }
            $stmt = $pdo->prepare('INSERT INTO la_request (date_requested,date_received,request_number,section,request_category,nature_of_request,details,requestor,received_by,imp_date,approval_date,accomplished_by,date_done,status) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)');
            $stmt->execute([$dr, $drx, $rn, $sec, $cat, $nor, $det, $req, $rcv, $imp, $apd, $acc, $ddn, $sts]);
            echo json_encode(['success' => true, 'record' => ['record_id' => (int) $pdo->lastInsertId(), 'date_requested' => $dr, 'date_received' => $drx, 'request_number' => $rn, 'section' => $sec, 'request_category' => $cat, 'nature_of_request' => $nor, 'details' => $det, 'requestor' => $req, 'received_by' => $rcv, 'imp_date' => $imp, 'approval_date' => $apd, 'accomplished_by' => $acc, 'date_done' => $ddn, 'status' => $sts]]);
            exit;
        ...
        case 'edit_record':
            ...
            $stmt = $pdo->prepare('UPDATE la_request SET date_requested=?,date_received=?,request_number=?,section=?,request_category=?,nature_of_request=?,details=?,requestor=?,received_by=?,imp_date=?,approval_date=?,accomplished_by=?,date_done=?,status=? WHERE record_id=?');
            $stmt->execute([$dr, $drx, $rn, $sec, $cat, $nor, $det, $req, $rcv, $imp, $apd, $acc, $ddn, $sts, $id]);
            echo json_encode(['success' => true, 'record' => ['record_id' => $id, 'date_requested' => $dr, 'date_received' => $drx, 'request_number' => $rn, 'section' => $sec, 'request_category' => $cat, 'nature_of_request' => $nor, 'details' => $det, 'requestor' => $req, 'received_by' => $rcv, 'imp_date' => $imp, 'approval_date' => $apd, 'accomplished_by' => $acc, 'date_done' => $ddn, 'status' => $sts]]);
            exit;
        ...
        case 'delete_record':
            ...
            $pdo->prepare('DELETE FROM la_request WHERE record_id=?')->execute([$id]);
...
try {
    $pdo->exec("ALTER TABLE la_request ADD COLUMN status VARCHAR(20) NULL DEFAULT NULL");
} catch (\Exception $e) { /* already exists */
}
// Migrate old remarks values into status for records that have remarks but no status
try {
    $pdo->exec("UPDATE la_request SET status = CASE
        WHEN LOWER(remarks) IN ('completed','complete') THEN 'Complete'
        WHEN LOWER(remarks) IN ('pending') THEN 'Pending'
        WHEN LOWER(remarks) IN ('on-going','on going','in-process','in process') THEN 'On-going'
        ELSE status END
    WHERE (status IS NULL OR status = '') AND remarks IS NOT NULL AND remarks <> ''");
} catch (\Exception $e) { /* migration failed gracefully */
}

// Normalize: use status if set, else fall back to remarks (old data), mapping old values to new
$allRecords = $pdo->query("
    SELECT record_id, date_requested, date_received, request_number, section,
           ...
           CASE
               ...
               WHEN remarks IN ('Completed','Complete','completed','complete') THEN 'Complete'
               WHEN remarks IN ('Pending','pending') THEN 'Pending'
               WHEN remarks IN ('On-going','On Going','on-going','In-Process','In Process','in-process') THEN 'On-going'
               ELSE status
           END AS status
    FROM la_request ORDER BY record_id DESC
")->fetchAll();
...
<head>
    ...
    <title>LA Monitoring Request – System</title>
    ...
    <style>
        ...
        .stat-card.active-filter.fc-teal2 {
            border-color: #696cff;
            box-shadow: 0 0 0 3px #696cff22;
        }
    ...
    <aside class="sidebar">
        ...
        <nav>
            <ul>
                ...
                <li class="nav-item nav-accordion open" id="Lasys">
                    <a class="nav-link active" href="#" onclick="toggleAcc('Lasys');return false;">
                    ...
                    <ul class="nav-sub">
                        <li><a class="nav-sub-link active" href="../LASYS/la_request.php">Monitoring Request</a></li>
                    </ul>
                </li>
    ...
    <div class="page-wrapper">
        ...
        <main class="main">
            <div class="breadcrumb">
                ...
                <a href="#">Label Assurance System</a><span><i class="fa fa-chevron-right"
                        style="font-size:9px;"></i></span>
                <span style="color:var(--text-mid);">Monitoring Request</span>
            </div>
            <?php if (!$isAdmin): ?>
            ...
            <div class="stat-grid">
                ...
                <div class="stat-card fc-green" id="sc-complete" onclick="filterByCard('complete')"
                    data-tip="Filter: Complete">
                    <div>
                ...
                <div class="stat-card fc-teal2" id="sc-ongoing" onclick="filterByCard('ongoing')"
                    data-tip="Filter: On-going">
                    <div>
                ...
                <div class="ce-modal-body">
                    ...
                    <div class="modal-field">
                        ...
                        <select class="modal-input" id="add-nature_of_request">
                            ...
                            <option value="Additional report name">Additional report name</option>
                            <option value="Change role">Change role</option>
                            <option value="Deactivate user">Deactivate user</option>
                            <option value="Inclusion of new market">Inclusion of new market</option>
                            <option value="Inclusion of product code">Inclusion of product code</option>
                            <option value="LASYS customization">LASYS customization</option>
                            <option value="LASYS installation">LASYS installation</option>
                            <option value="Modify data">Modify data</option>
                            <option value="Modify LI report design">Modify LI report design</option>
                            <option value="Reset password">Reset password</option>
                            <option value="Revision of forms">Revision of forms</option>
                            <option value="Inclusion of LMI">Inclusion of LMI</option>
                        </select>
                    </div>
                    ...
                    <div class="form-section"><i class="fa-solid fa-images"></i> Attachments / Photos</div>
                    <div class="modal-field">
                        <label class="modal-label">Upload Photos <span style="color:#9aa3b0;font-weight:400;">(optional, max
                                5 files)</span></label>
                        <div class="photo-upload-area" id="add-photo-area"
                            onclick="document.getElementById('add-photos').click()" ondragover="photoDragOver(event,'add')"
                            ondragleave="photoDragLeave(event,'add')" ondrop="photoDrop(event,'add')">
                            <i class="fa-solid fa-cloud-arrow-up"
                                style="font-size:28px;color:var(--acc);margin-bottom:8px;"></i>
                            <div style="font-size:13px;font-weight:600;color:#4a5568;">Click to upload or drag &amp; drop
                            </div>
                            <div style="font-size:11px;color:#9aa3b0;margin-top:4px;">PNG, JPG, GIF up to 5MB each</div>
                        </div>
                        <input type="file" id="add-photos" multiple accept="image/*" style="display:none;"
                            onchange="previewPhotos('add')">
                        <div class="photo-preview-grid" id="add-photo-previews"></div>
                    </div>
                </div>
                ...
                <div class="ce-modal-body">
                    ...
                    <div class="modal-field">
                        ...
                        <div class="photo-upload-area" id="edit-photo-area"
                            ...
                            ondragover="photoDragOver(event,'edit')" ondragleave="photoDragLeave(event,'edit')"
                            ondrop="photoDrop(event,'edit')">
                            <i class="fa-solid fa-cloud-arrow-up"
                                style="font-size:28px;color:#ff9f43;margin-bottom:8px;"></i>
                            <div style="font-size:13px;font-weight:600;color:#4a5568;">Click to upload or drag &amp; drop
                            </div>
                            <div style="font-size:11px;color:#9aa3b0;margin-top:4px;">PNG, JPG, GIF up to 5MB each</div>
                        </div>
                        <input type="file" id="edit-photos" multiple accept="image/*" style="display:none;"
                            onchange="previewPhotos('edit')">
                        <div class="photo-preview-grid" id="edit-photo-previews"></div>
                    </div>
                </div>
                <div class="ce-modal-footer">
    ...
    <div class="view-modal-overlay" id="view-modal">
        <div class="view-modal">
            ...
            <div class="view-grid">
                ...
                <div class="view-field full"><label>Nature of Request</label><span id="view-nature_of_request"></span>
                </div>
                ...
                <div class="view-field full"><label>Details</label><span id="view-details"></span></div>
                <div class="view-field full"><label>Status</label><span id="view-status"></span></div>
            </div>
    ...
    <script>
        ...
        function getFiltered() {
            ...
            return allRecords.filter(r => {
                const ms = !q || [r.request_number, r.section, r.request_category, r.nature_of_request, r.requestor, r.received_by, r.accomplished_by, r.details, r.status, r.date_requested].some(f => (f || '').toLowerCase().includes(q));
                let mf = true;
...
<tr>
  ...
  <td><span class="la-badge"><i class="fa-solid fa-hashtag" style="font-size:10px;"></i> ${r.request_number || '—'}</span></td>
  ...
  <td>${(() => { const ns = normStatus(r); const s = r.status || ''; const sc = ns === 'complete' ? '#28c76f' : ns === 'on-going' ? '#696cff' : ns === 'pending' ? '#ff9f43' : '#9aa3b0'; const label = ns === 'complete' ? 'Complete' : ns === 'on-going' ? 'On-going' : ns === 'pending' ? 'Pending' : s; return label ? `<span style="display:inline-block;padding:2px 10px;border-radius:20px;font-size:11px;font-weight:700;background:${sc}22;color:${sc};">${label}</span>` : '<span style="color:#ccc;font-size:12px;">—</span>'; })()}</td>
  <td><div class="action-cell">
    ...
    <button class="action-btn del" title="Delete" onclick='openDelModal(${r.record_id},"${(r.request_number || "Record #" + r.record_id).replace(/"/g, "")}")'><i class="fa fa-trash"></i></button>` : ''}
        ...
        function normStatus(r) {
            ...
            if (['on-going', 'on going', 'in-process', 'in process'].includes(s)) return 'on-going';
        ...
        function openViewModal(id) {
            ...
            const stLabel = ns === 'complete' ? 'Complete' : ns === 'on-going' ? 'On-going' : ns === 'pending' ? 'Pending' : (r.status || '—');
            const stColor = ns === 'complete' ? '#28c76f' : ns === 'on-going' ? '#696cff' : ns === 'pending' ? '#ff9f43' : null;
            const stEl = document.getElementById('view-status');
            if (stColor) {
                stEl.innerHTML = `<span style="display:inline-block;padding:3px 12px;border-radius:20px;font-size:12px;font-weight:700;background:${stColor}22;color:${stColor};">${stLabel}</span>`;
            } else { stEl.textContent = stLabel; }
        ...
        function openAddModal() {
            ...
            ['add-date_requested', 'add-date_received', 'add-section', 'add-request_category', 'add-nature_of_request', 'add-details', 'add-requestor', 'add-imp_date', 'add-approval_date', 'add-date_done', 'add-status'].forEach(id => document.getElementById(id).value = '');
            ['add-received_by', 'add-accomplished_by'].forEach(id => document.getElementById(id).value = '');
            _photoFiles['add'] = []; renderPhotoPreviews('add');
        ...
        function submitAddRecord() {
            const flds = ['date_requested', 'date_received', 'request_number', 'section', 'request_category', 'nature_of_request', 'details', 'requestor', 'received_by', 'imp_date', 'approval_date', 'accomplished_by', 'date_done', 'status'];
            ...
            fetch(window.location.pathname, { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: new URLSearchParams({ __action: 'add_record', ...data }).toString() })
                .then(r => r.json()).then(d => {
                    if (d.success) {
                        ...
                        showResult('success', 'Request Added!', 'LA request has been saved.', [
        ...
        function openEditModal(id) {
            ...
            const flds = ['date_requested', 'date_received', 'request_number', 'section', 'request_category', 'nature_of_request', 'details', 'requestor', 'received_by', 'imp_date', 'approval_date', 'accomplished_by', 'date_done', 'status'];
            flds.forEach(f => { const el = document.getElementById('edit-' + f); if (el) el.value = r[f] || ''; });
            _photoFiles['edit'] = []; renderPhotoPreviews('edit');
        ...
        function submitEditRecord() {
            ...
            const flds = ['date_requested', 'date_received', 'request_number', 'section', 'request_category', 'nature_of_request', 'details', 'requestor', 'received_by', 'imp_date', 'approval_date', 'accomplished_by', 'date_done', 'status'];
            const data = {}; flds.forEach(f => { const el = document.getElementById('edit-' + f); data[f] = el ? el.value.trim() : ''; });
            ...
            fetch(window.location.pathname, { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: new URLSearchParams({ __action: 'edit_record', id, ...data }).toString() })
                .then(r => r.json()).then(d => {
                    if (d.success) {
                        ...
                        showResult('success', 'Request Updated!', 'LA request has been updated.', [['📋 Request No.', d.record.request_number], ['📁 Section', d.record.section || '—']]);
        ...
        function exportCSV() {
            const rows = [['Record ID', 'Request No.', 'Section', 'Category', 'Nature of Request', 'Requestor', 'Received By', 'Accomplished By', 'Date Requested', 'Date Received', 'Imp Date', 'Approval Date', 'Date Done', 'Details', 'Status']];
            getFiltered().forEach(r => rows.push([r.record_id, r.request_number || '', r.section || '', r.request_category || '', r.nature_of_request || '', r.requestor || '', r.received_by || '', r.accomplished_by || '', r.date_requested || '', r.date_received || '', r.imp_date || '', r.approval_date || '', r.date_done || '', r.details || '', r.status || '']));
            const csv = rows.map(r => r.map(v => `"${String(v).replace(/"/g, '""')}"`).join(',')).join('\n');
            const a = document.createElement('a'); a.href = URL.createObjectURL(new Blob([csv], { type: 'text/csv' })); a.download = 'la_requests.csv'; a.click();
            showToast('Exported!', 'success');
        }
        function toggleAcc(id) { document.getElementById(id).classList.toggle('open'); }
        /* ── Photo Upload ── */
        const _photoFiles = { add: [], edit: [] };

        function previewPhotos(prefix) {
            const input = document.getElementById(prefix + '-photos');
            const newFiles = Array.from(input.files);
            newFiles.forEach(f => {
                if (_photoFiles[prefix].length >= 5) return;
                if (f.size > 5 * 1024 * 1024) { showToast('File too large: ' + f.name, 'error'); return; }
                _photoFiles[prefix].push(f);
            });
            input.value = '';
        ...
        function renderPhotoPreviews(prefix) {
            const grid = document.getElementById(prefix + '-photo-previews');
            if (!grid) return;
            grid.innerHTML = _photoFiles[prefix].map((f, i) => {
                const url = URL.createObjectURL(f);
                return `<div class="photo-thumb" id="${prefix}-thumb-${i}">
                    <img src="${url}" alt="${f.name}">
                    <div class="photo-thumb-name">${f.name}</div>
                    <button class="photo-thumb-remove" onclick="removePhoto('${prefix}',${i})" title="Remove"><i class="fa fa-times"></i></button>
                </div>`;
            }).join('');
        }

        function removePhoto(prefix, idx) {
            _photoFiles[prefix].splice(idx, 1);
            renderPhotoPreviews(prefix);
        }

        function photoDragOver(e, prefix) { e.preventDefault(); document.getElementById(prefix + '-photo-area').classList.add('drag-over'); }
        function photoDragLeave(e, prefix) { document.getElementById(prefix + '-photo-area').classList.remove('drag-over'); }
        function photoDrop(e, prefix) {
            e.preventDefault();
            document.getElementById(prefix + '-photo-area').classList.remove('drag-over');
