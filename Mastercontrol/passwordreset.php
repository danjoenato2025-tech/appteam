...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['__action'])) {
    ...
    switch ($_POST['__action']) {
        ...
        case 'get_next_request_number':
            $year = date('y');
            $prefix = "MC-PR-{$year}-";
            $stmt = $pdo->prepare("SELECT request_number FROM passwordreset_record WHERE request_number LIKE ? ORDER BY record_id DESC LIMIT 1");
            $stmt->execute([$prefix . '%']);
        ...
        case 'add_record':
            ...
            $fn = trim($_POST['full_name'] ?? '') ?: null;
            $rs = trim($_POST['reason'] ?? '') ?: null;
            $sec = trim($_POST['section'] ?? '') ?: null;
            $dep = trim($_POST['department'] ?? '') ?: null;
            $pf = trim($_POST['performed'] ?? '') ?: null;
            $dor = trim($_POST['date_of_reset'] ?? '') ?: null;
            $rmk = trim($_POST['remarks'] ?? '') ?: null;
            if (!$dr || !$rn) {
                echo json_encode(['success' => false, 'message' => 'Date Requested and Request Number are required.']);
                exit;
            }
            $stmt = $pdo->prepare('INSERT INTO passwordreset_record (date_requested,date_received,request_number,associate_number,full_name,reason,section,department,performed,date_of_reset,remarks) VALUES (?,?,?,?,?,?,?,?,?,?,?)');
            $stmt->execute([$dr, $drx, $rn, $an, $fn, $rs, $sec, $dep, $pf, $dor, $rmk]);
            echo json_encode(['success' => true, 'record' => ['record_id' => (int) $pdo->lastInsertId(), 'date_requested' => $dr, 'date_received' => $drx, 'request_number' => $rn, 'associate_number' => $an, 'full_name' => $fn, 'reason' => $rs, 'section' => $sec, 'department' => $dep, 'performed' => $pf, 'date_of_reset' => $dor, 'remarks' => $rmk]]);
            exit;
        ...
        case 'edit_record':
            ...
            $stmt = $pdo->prepare('UPDATE passwordreset_record SET date_requested=?,date_received=?,request_number=?,associate_number=?,full_name=?,reason=?,section=?,department=?,performed=?,date_of_reset=?,remarks=? WHERE record_id=?');
            $stmt->execute([$dr, $drx, $rn, $an, $fn, $rs, $sec, $dep, $pf, $dor, $rmk, $id]);
            echo json_encode(['success' => true, 'record' => ['record_id' => $id, 'date_requested' => $dr, 'date_received' => $drx, 'request_number' => $rn, 'associate_number' => $an, 'full_name' => $fn, 'reason' => $rs, 'section' => $sec, 'department' => $dep, 'performed' => $pf, 'date_of_reset' => $dor, 'remarks' => $rmk]]);
            exit;
        ...
        case 'delete_record':
            ...
            $pdo->prepare('DELETE FROM passwordreset_record WHERE record_id=?')->execute([$id]);
...
$allRecords = $pdo->query('SELECT record_id,date_requested,date_received,request_number,associate_number,full_name,reason,section,department,performed,date_of_reset,remarks FROM passwordreset_record ORDER BY record_id DESC')->fetchAll();
...
<head>
    ...
    <title>Password Request – System</title>
...
<body>
            ...
            <ul>
                ...
                <li class="nav-item nav-accordion open" id="masterControl">
                    ...
                    <ul class="nav-sub">
                        ...
                        <li><a class="nav-sub-link active" href="passwordreset.php">Password Request</a></li>
    ...
    <div class="page-wrapper">
        ...
        <main class="main">
            <div class="breadcrumb"><a href="#">Home</a><span><i class="fa fa-chevron-right"
                    ...
                    style="color:var(--text-mid);">Password Request</span></div>
            <?php if (!$isAdmin): ?>
            ...
            <div class="stat-grid">
                ...
                <div class="stat-card fc-blue" id="sc-month" onclick="filterByCard('month')"
                    ...
                    <div class="stat-icon sky"><i class="fa-solid fa-calendar-week"></i></div>
                </div>
            </div>
    ...
    <?php if ($isAdmin): ?>
        ...
        <div class="ce-modal-overlay" id="add-modal">
            <div class="ce-modal">
                <div class="ce-modal-header" style="background:linear-gradient(135deg,#2196f3,#1565c0);">
                    <div class="ce-modal-header-icon"><i class="fa-solid fa-key"></i></div>
                    <div>
                        <h3>Add Password Request</h3>
                ...
                <div class="ce-modal-body">
                    ...
                    <div class="modal-grid2">
                        <div><label class="modal-label">Associate No.</label><input class="modal-input"
                                id="add-associate_number" type="text" placeholder="e.g. 2204014"></div>
                        <div><label class="modal-label">Full Name</label><input class="modal-input" id="add-full_name"
                                type="text" placeholder="Full Name"></div>
                    </div>
                    <div class="modal-grid2">
                        <div><label class="modal-label">Section</label><input class="modal-input" id="add-section"
                                type="text" placeholder="e.g. ICT"></div>
                        <div><label class="modal-label">Department</label><input class="modal-input" id="add-department"
                                type="text" placeholder="e.g. Warehouse"></div>
                    </div>
                    <div class="modal-field"><label class="modal-label">Reason</label><textarea class="modal-input"
                            id="add-reason" rows="3" placeholder="Reason for password request..."></textarea></div>
                    ...
                    <div class="modal-grid2">
                        ...
                        <div><label class="modal-label">Date of Reset</label><input class="modal-input"
                                id="add-date_of_reset" type="date"></div>
                    </div>
                    <div class="modal-field"><label class="modal-label">Remarks</label><textarea class="modal-input"
                            id="add-remarks" rows="2" placeholder="Additional remarks..."></textarea></div>
                </div>
                <div class="ce-modal-footer">
                    <button class="btn-modal-cancel" onclick="closeAddModal()">Cancel</button>
                    <button class="btn-modal-submit" style="background:linear-gradient(135deg,#2196f3,#1565c0);"
            ...
            <div class="ce-modal">
                ...
                <div class="ce-modal-body">
                    ...
                    <div class="modal-grid2">
                        ...
                        <div><label class="modal-label">Full Name</label><input class="modal-input" id="edit-full_name"
                                type="text"></div>
                    </div>
                    <div class="modal-grid2">
                        <div><label class="modal-label">Section</label><input class="modal-input" id="edit-section"
                                type="text"></div>
                        <div><label class="modal-label">Department</label><input class="modal-input" id="edit-department"
                                type="text"></div>
                    </div>
                    <div class="modal-field"><label class="modal-label">Reason</label><textarea class="modal-input"
                            id="edit-reason" rows="3"></textarea></div>
                    <div class="form-section"><i class="fa-solid fa-user-check"></i> Action</div>
                    <div class="modal-grid2">
                        <div>
                            <label class="modal-label">Performed By</label>
                        ...
                        <div><label class="modal-label">Date of Reset</label><input class="modal-input"
                                id="edit-date_of_reset" type="date"></div>
                    </div>
                    <div class="modal-field"><label class="modal-label">Remarks</label><textarea class="modal-input"
                            id="edit-remarks" rows="2"></textarea></div>
                </div>
                <div class="ce-modal-footer">
    ...
    <div class="view-modal-overlay" id="view-modal">
        <div class="view-modal">
            <div class="view-modal-header">
                <div class="view-icon-wrap"><i class="fa-solid fa-key"></i></div>
                <div>
                    <div class="view-name" id="view-request_number">—</div>
                    <div class="view-sub" id="view-full_name_sub">—</div>
                </div>
            </div>
            <div class="view-grid">
                ...
                <div class="view-field"><label>Associate No.</label><span id="view-associate_number"></span></div>
                <div class="view-field"><label>Full Name</label><span id="view-full_name"></span></div>
                ...
                <div class="view-field"><label>Date of Reset</label><span id="view-date_of_reset"></span></div>
...
<tr>
  ...
  <td style="font-size:13.5px;color:var(--text-mid);">${r.full_name || '—'}</td>
  ...
  <td style="font-size:13px;color:var(--text-mid);">${r.date_of_reset || '—'}</td>
  <td><div class="action-cell">
    ...
    <button class="action-btn del" title="Delete" onclick='openDelModal(${r.record_id},"${(r.request_number || "Record #" + r.record_id).replace(/"/g, "")}")'><i class="fa fa-trash"></i></button>` : ''}
        ...
        function _highlightCard(type) {
            const map = { done: 'sc-done', pending: 'sc-pending', today: 'sc-today', month: 'sc-month' };
        ...
        function openViewModal(id) {
            ...
            document.getElementById('view-full_name_sub').textContent = r.full_name || '—';
            ...
            document.getElementById('view-full_name').textContent = r.full_name || '—';
            ...
            document.getElementById('view-date_of_reset').textContent = r.date_of_reset || '—';
        ...
        function openAddModal() {
            ...
            ['add-date_requested', 'add-date_received', 'add-associate_number', 'add-full_name', 'add-reason', 'add-section', 'add-department', 'add-date_of_reset', 'add-remarks'].forEach(id => document.getElementById(id).value = '');
        ...
        function submitAddRecord() {
            ...
            const date_of_reset = document.getElementById('add-date_of_reset').value.trim();
            ...
            fetch(window.location.pathname, {
                ...
                body: new URLSearchParams({ __action: 'add_record', date_requested, date_received, request_number, associate_number, full_name, reason, section, department, performed, date_of_reset, remarks }).toString()
            })
                .then(r => r.json()).then(d => {
                    if (d.success) {
                        allRecords.unshift(d.record); curPage = 1; renderTable(); closeAddModal();
                        showResult('success', 'Record Added!', 'Password request has been saved.', [['🔑 Request No.', d.record.request_number], ['👤 Name', d.record.full_name || '—'], ['🏢 Section', d.record.section || '—'], ['👨‍💼 Performed', d.record.performed || '—']]);
                    }
        ...
        function openEditModal(id) {
            ...
            document.getElementById('edit-full_name').value = r.full_name || '';
            ...
            document.getElementById('edit-date_of_reset').value = r.date_of_reset || '';
        ...
        function submitEditRecord() {
            ...
            const date_of_reset = document.getElementById('edit-date_of_reset').value.trim();
            ...
            fetch(window.location.pathname, {
                method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                body: new URLSearchParams({ __action: 'edit_record', id, date_requested, date_received, request_number, associate_number, full_name, reason, section, department, performed, date_of_reset, remarks }).toString()
            })
                .then(r => r.json()).then(d => {
                    if (d.success) {
                        ...
                        showResult('success', 'Record Updated!', 'Password request has been updated.', [['🔑 Request No.', d.record.request_number], ['👤 Name', d.record.full_name || '—']]);
                    }
        ...
        function exportCSV() {
            const rows = [['Record ID', 'Request No.', 'Associate No.', 'Full Name', 'Section', 'Department', 'Date Requested', 'Date Received', 'Performed', 'Date of Reset', 'Reason', 'Remarks']];
            getFiltered().forEach(r => rows.push([r.record_id, r.request_number || '', r.associate_number || '', r.full_name || '', r.section || '', r.department || '', r.date_requested || '', r.date_received || '', r.performed || '', r.date_of_reset || '', r.reason || '', r.remarks || '']));
            const csv = rows.map(r => r.map(v => `"${String(v).replace(/"/g, '""')}"`).join(',')).join('\n');
            const a = document.createElement('a'); a.href = URL.createObjectURL(new Blob([csv], { type: 'text/csv' })); a.download = 'passwordreset_records.csv'; a.click();
